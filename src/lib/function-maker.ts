const {sin, PI : pi, E : e, cos, tan, pow, sqrt, exp, log10, log2, log, log1p, expm1, cosh, sinh, tanh, acosh, asinh, atanh, hypot, trunc, cbrt, round, abs, acos, asin, atan2 : atan} = Math
...
export function functionMaker(fn : string) {
    return eval(`(x, slider) => { return (${fn.replaceAll('\n', '+')})*0.1 }`) as (x : number, slider : number) => number
}