...
function delayerGenerator(fn : (x : number, slider : number) => number, slider : number) {
...
class CustomProcessor extends AudioWorkletProcessor {
            ...
            if (event.data.fn && typeof event.data.slider === "number") {
                try {
                    ...
                    if (typeof fn(5, 5) === "number") {
                        ...
                        this.port.postMessage({error: 'Error: Script does not take and output a number'});
                    }
                } catch (e) {
    ...
    process(inputs: Float32Array[][], outputs: Float32Array[][], parameters: Record<string, Float32Array>): boolean {
        ...
        if (!this.changed) {
            ...
            const firstBitOfNextProcess = this.fn((this.changed*outputChannel.length + this.phase)/sampleRate, this.slider);
